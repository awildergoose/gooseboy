...
public class WasmInputManager {
	...
	public static int getMouseXInFramebuffer() {
		...
		double mouseX = Minecraft.getInstance().mouseHandler.xpos() * (double) width / Minecraft.getInstance().getWindow().getScreenWidth();
		int fbX = (int) (mouseX - (((width - Gooseboy.FRAMEBUFFER_WIDTH) / 2) + 5));
		if (fbX < 0) fbX = 0;
	...
	public static int getMouseYInFramebuffer() {
		...
		double mouseY = Minecraft.getInstance().mouseHandler.ypos() * (double) height / Minecraft.getInstance().getWindow().getScreenHeight();
		int fbY = (int) (mouseY - (((height - Gooseboy.FRAMEBUFFER_HEIGHT) / 2) + 5));
		if (fbY < 0) fbY = 0;
